...
/**
 * Create an bootstrap menu item. Automatically sets .active on current route.
 *
 * @param  string  $route
 * @param  string  $name
 * @param  string  $class
 * @return string
 */
Form::macro('navitem', function($route, $name, $class = '')
{
    if (URL::route($route) != Request::url())
    {
        return '<li class="'.$class.'"><a href="'.URL::route($route).'">'.e($name).'</a></li>';
    }
    else
    {
        return '<li class="active '.$class.'"><a href="'.URL::route($route).'">'.e($name).'</a></li>';
    }
});
...
if ( ! function_exists('binary_encode'))
{
     ...
     * Based on http://home.paulschou.net/tools/xlate/
     *
     * @param  string  $value
     * @return string
     */
...
if ( ! function_exists('integer_to_short_string'))
{
     ...
     * Based on http://briancray.com/posts/free-php-url-shortener-script/
     *
...
if ( ! function_exists('cache_buster'))
{
    ...
    function cache_buster($file)
    {
        ...
        if ($files->exists($path))
        {
            ...
            return URL::to($filename.'.'.$buster.'.'.$ext);
        }
